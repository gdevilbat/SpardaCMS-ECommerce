...
class LazadaController extends CoreController
{
    public function authentication(Request $request)
    {
        return MarketPlace::driver('lazada')->shop->getAuthUrl(url(action('\\'.Self::class.'@callback').'?'.http_build_query(['callback' => $request->input('callback')])));
    }

    public function callback(Request $request)
    {
        $data =  MarketPlace::driver('lazada')->shop->requestToken($request->code);

        $config = Setting::where('name', 'lazada_id')->first();
        if(empty($config))
            $config = new Setting;

        $config->name = 'lazada_id';
        $config->value = $data->country_user_info[0]->seller_id;
        $config->save();
