...
class CategoryControllerTest extends TestCase
{
    ...
    {
        $response = $this->get(action('\Gdevilbat\SpardaCMS\Modules\Post\Http\Controllers\CategoryController@index'));
        ...
        $response = $this->actingAs($user)
                         ->from(action('\Gdevilbat\SpardaCMS\Modules\Post\Http\Controllers\CategoryController@index'))
                         ->json('GET',action('\Gdevilbat\SpardaCMS\Modules\Post\Http\Controllers\CategoryController@serviceMaster'))
                         ->assertSuccessful()
    ...
    {
        $response = $this->post(action('\Gdevilbat\SpardaCMS\Modules\Post\Http\Controllers\CategoryController@store'));
        ...
        $response = $this->actingAs($user)
                         ...
                         ->assertStatus(302)
                         ->assertRedirect(action('\Gdevilbat\SpardaCMS\Modules\Post\Http\Controllers\CategoryController@create'))
        ...
        $this->assertDatabaseHas(\Gdevilbat\SpardaCMS\Modules\Taxonomy\Entities\TermTaxonomy::getTableName(), ['term_id' => $term->getKey(), 'taxonomy' => 'category']);
    }
    ...
    {
        $response = $this->post(action('\Gdevilbat\SpardaCMS\Modules\Post\Http\Controllers\CategoryController@destroy'), [
                        '_method' => 'DELETE'
                    ]);
        ...
        $response = $this->actingAs($user)
                        ->from(action('\Gdevilbat\SpardaCMS\Modules\Post\Http\Controllers\CategoryController@index'))
                        ->post(action('\Gdevilbat\SpardaCMS\Modules\Post\Http\Controllers\CategoryController@destroy'), [
                            $taxonomy->getKeyName() => encrypt($taxonomy->getKey()),
                            '_method' => 'DELETE'
                        ])
                        ->assertStatus(302)
